...
import {
  createGestionnaireReleveResourceAction,
  getGestionnaireReleveResourceAction,
  updateGestionnaireReleveResourceAction,
...
export default function GestionnaireReleveResourceForm({
  mode,
  editingId,
  sectionSlug,
  sectionDesignation,
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      try {
        const detail = await getGestionnaireReleveResourceAction({ sectionSlug, id: editingId });
        setForm({
  ...
  const submit = useCallback(() => {
    ...
    startTransition(async () => {
      try {
        ...
        if (mode === "create") {
          await createGestionnaireReleveResourceAction(payload);
        } else if (editingId) {
          await updateGestionnaireReleveResourceAction({ ...payload, id: editingId });
        }
        onSaved();
      } catch (e) {
  ...
  return (
    <ResourceMultiStepFormShell
      headerTitle={mode === "create" ? "Nouveau relevé de cotes" : "Modifier le relevé"}
      mode={mode}
      steps={STEPS}
      activeStep={step}
      error={error}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <input
              ...
              placeholder="Ex. Relevé de notes annuel"
            />
          </div>
          <div>
