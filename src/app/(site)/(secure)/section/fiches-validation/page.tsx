...
import { listGestionnaireValidationResourcesAction, type ValidationResourceRow } from "@/actions/gestionnaireValidationResources";
import GestionnaireValidationResourcesClient from "./GestionnaireValidationResourcesClient";

export const metadata: Metadata = {
  title: "Fiches de validation | INBTP",
};

export default async function GestionnaireFichesValidationPage() {
  ...
  let initialData: { rows: ValidationResourceRow[]; total: number; page: number; limit: number } = {
    rows: [],
    total: 0,
    page: 1,
    limit: 10,
  ...
  try {
    initialData = await listGestionnaireValidationResourcesAction({
  ...
  return (
    <GestionnaireValidationResourcesClient
      sectionSlug={scope.sectionSlug}
