...
import OrganisateurStageResourcesClient from "./OrganisateurStageResourcesClient";
...
export const metadata: Metadata = {
  title: "Ressources stages | INBTP",
};

export default async function OrganisateurRessourcesStagesPage() {
  ...
  if (!sectionCtx) {
    return (
      <div className="mx-auto max-w-xl rounded-xl border border-amber-200 bg-amber-50/80 p-6 text-sm text-amber-950 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-100">
        <p className="font-semibold">Accès réservé au chargé de recherche ou au chargé d&apos;enseignement</p>
        <p className="mt-2 text-amber-900/90 dark:text-amber-100/90">
          Vous devez être désigné comme <strong>chargé de recherche</strong> ou{" "}
          <strong>chargé d&apos;enseignement</strong> du bureau d&apos;une section pour gérer les ressources
          stages (volet recherche). Rôle actuel : organisateur sans l&apos;une de ces affectations sur la section.
        </p>
      </div>
    );
  }

  let initialData: { rows: StageResourceRow[]; total: number; page: number; limit: number } = {
    rows: [],
    total: 0,
    page: 1,
    limit: 10,
  ...
  try {
    initialData = await listOrganisateurStageResourcesAction({
  ...
  return (
    <OrganisateurStageResourcesClient
      sectionSlug={sectionCtx.sectionSlug}
