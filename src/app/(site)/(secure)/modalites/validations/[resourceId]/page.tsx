...
export const metadata: Metadata = {
  title: "Vérification paiement - Validation | INBTP",
};

type PageProps = { params: Promise<{ resourceId: string }> };

export default async function ModalitesValidationCommandesPage({ params }: PageProps) {
  ...
  try {
    const detail = await getGestionnaireValidationResourceAction({ sectionSlug: scope.sectionSlug, id: rid });
  ...
  return (
    <EtudiantResourceCommandesClient
      ...
      toolbarDescription="Vérifiez les paiements des étudiants pour cette fiche de validation avant de valider leur accès."
    />
  );
}
