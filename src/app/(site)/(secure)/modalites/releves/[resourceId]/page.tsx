...
export const metadata: Metadata = {
  title: "Vérification paiement - Relevé | INBTP",
};

type PageProps = { params: Promise<{ resourceId: string }> };

export default async function ModalitesReleveCommandesPage({ params }: PageProps) {
  ...
  try {
    const detail = await getGestionnaireReleveResourceAction({ sectionSlug: scope.sectionSlug, id: rid });
  ...
  return (
    <EtudiantResourceCommandesClient
      ...
      toolbarDescription="Vérifiez les paiements des étudiants pour ce relevé de cotes avant de valider leur accès."
    />
  );
}
