...
export const metadata: Metadata = {
  title: "Demandes - Validations | INBTP",
};
...
export default async function DemandesValidationsPage() {
  ...
  try {
    const resourcesData = await listGestionnaireValidationResourcesAction({
    ...
    const resourcesWithDemandes = await Promise.all(
      resourcesData.rows.map(async (r: ValidationResourceRow) => {
  ...
  return (
    <DemandesRessourcesClient
      type="validation"
      typeLabel="Validations"
      typeIcon="solar:checklist-bold-duotone"
      sectionSlug={scope.sectionSlug}
      sectionDesignation={scope.sectionDesignation}
      resources={resources}
      initialError={initialError}
      detailPath="/section/fiches-validation/validations"
    />
  );
}
