...
export default function CourseDetailFromSessionPage() {
  ...
  if (!course) {
    return (
      <>
        <Breadcrumb
          pageName="Cours indisponible"
          pageDescription="Les données du cours ne sont plus disponibles pour cette session."
  ...
  return (
    <>
      <Breadcrumb
        pageName={course.matiereDesignation}
        pageDescription={`${course.programmeDesignation} — ${course.uniteCode} ${course.uniteDesignation}`}
        trail={[
          { label: "Études", href: "/etudes" },
          {
            label: "Faculté",
            href: course.sectionSlug ? `/etudes/${course.sectionSlug}` : "/etudes",
          },
        ]}
      />
      <main className="mx-auto w-full max-w-4xl px-4 py-10 sm:px-6">
        <article className="rounded-2xl border border-slate-300 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-darklight">
          ...
          <h2 className="mt-2 text-3xl font-black text-blue-950 dark:text-white">{course.matiereDesignation}</h2>
