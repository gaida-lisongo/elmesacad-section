...
export async function POST(request: Request) {
  ...
  if (!ALLOWED.has(file.type)) {
    return NextResponse.json(
      { message: "Format accepté : JPEG, PNG, WebP, GIF, SVG" },
      { status: 400 }
    );
  }
  ...
  const filename = `formation-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
  const schema = process.env.IMAGE_FORMATION_UPLOAD_SCHEMA ?? "formations";
