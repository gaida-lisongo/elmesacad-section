...
const COMMANDE_STAGE_CREATE_PATH = "/commandes";
...
export type MarketplaceStageOrderCreatePayload = {
  ...
  reference: string;
  type: "stage";
} & OrderStageStudentPayload;

export type SubmitOrderStageResearchResult =
 ...
 */
export async function submitOrderStageResearchAction(input: {
  localCommandeId: string;
  payload: OrderStageStudentPayload;
}): Promise<SubmitOrderStageResearchResult> {
  const localId = String(input.localCommandeId ?? "").trim();
  const { payload } = input;

  console.log("[stage][submit] début POST /commandes", { localCommandeId: localId, path: COMMANDE_STAGE_CREATE_PATH });
  ...
  const body: MarketplaceStageOrderCreatePayload = {
    parcoursId,
    ressourceId,
    ...
    recipientQuality: payload.recipientQuality,
    companyName: payload.companyName,
    companyLocation: payload.companyLocation,
    documentReference: payload.documentReference,
  };

  console.log("[stage][submit] corps (aperçu)", {
    ...body,
  });

  let res: Response;
  try {
    res = await fetchEtudiantApi(COMMANDE_STAGE_CREATE_PATH, {
      method: "POST",
      headers: {
