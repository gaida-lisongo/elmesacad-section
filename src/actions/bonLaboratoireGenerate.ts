...
import type { DocumentBonLaboratoirePayload } from "@/lib/paiement/documentBonLaboratoirePayload";

/** Route du microservice étudiant pour le PDF bon de laboratoire. */
const BON_LABORATOIRE_GENERATE_PATH = "/laboratoire/generate";

export type GenerateBonLaboratoirePdfResult =
...
function stemFromPayload(payload: DocumentBonLaboratoirePayload): string {
  const name = payload.promotion?.studentName?.trim() || "bon-laboratoire";
  return name
    .replace(/[^\w\s-]/g, "")
    .trim()
    .replace(/\s+/g, "-")
...
export async function generateBonLaboratoirePdfAction(
  payload: DocumentBonLaboratoirePayload
): Promise<GenerateBonLaboratoirePdfResult> {
  let res: Response;
  try {
    res = await fetchEtudiantApi(BON_LABORATOIRE_GENERATE_PATH, {
      method: "POST",
      headers: {
  ...
  if (res.ok && contentType.includes("application/pdf")) {
    ...
    if (!filename.toLowerCase().endsWith(".pdf")) {
      const stem = filename.replace(/\.pdf$/i, "") || safeStem || "bon-laboratoire";
      filename = `${stem}.pdf`;
    }
