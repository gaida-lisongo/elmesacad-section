...
/** Schéma Zod service étudiant : `categorie: "stage"` → `titulaire` (userRef), pas `lecteurs`. */
...
async function assertStageBureauContext() {
...
function assertPayloadResourceIsStage(raw: unknown) {
  const r = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
  const cat = String(r.categorie ?? "").toLowerCase();
  if (cat && cat !== "stage") {
    throw new Error("Cette ressource n'est pas une ressource de catégorie « stage ».");
  }
}

function rowFromApi(raw: unknown): StageResourceRow | null {
  const r = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
...
export async function listOrganisateurStageResourcesAction(input: {
  sectionSlug: string;
  page?: number;
  limit?: number;
  search?: string;
}): Promise<{ rows: StageResourceRow[]; total: number; page: number; limit: number }> {
  const ctx = await assertStageBureauContext();
  ...
  if (!upstream.ok) {
    throw new Error(pickErrorMessage(payload, "Impossible de charger les ressources stages."));
  }
  ...
  const rows = data.map(rowFromApi).filter((x): x is StageResourceRow => x != null);
  return {
    rows,
    total,
...
export async function listStageCommandesForResourceAction(input: {
  sectionSlug: string;
  resourceId: string;
  page?: number;
  limit?: number;
  ...
  assertPayloadResourceIsStage(verifyRow);
  ...
  const criteria = JSON.stringify({ ressourceId: resourceId, type: "stage" });
  const sp = new URLSearchParams({
    page: String(page),
    limit: String(limit),
    type: "stage",
...
export async function getOrganisateurStageResourceAction(input: {
  sectionSlug: string;
  id: string;
}): Promise<
  StageResourceRow & {
...
> {
  ...
  assertPayloadResourceIsStage(row);
...
export async function createOrganisateurStageResourceAction(input: {
  sectionSlug: string;
  designation: string;
  descriptionSections: DescriptionSectionInput[];
  amount: number;
  ...
  /* POST aligné sur `resourceStageSchema` (Zod) : `titulaire`, `description`, `status: inactive` — pas `lecteurs`. */
  const body = {
    categorie: "stage" as const,
  ...
  try {
    row = await patchOrganisateurStageResourceStatusAction({
...
export async function updateOrganisateurStageResourceAction(input: {
  sectionSlug: string;
  id: string;
  designation: string;
  descriptionSections: DescriptionSectionInput[];
  ...
  await getOrganisateurStageResourceAction({ sectionSlug: ctx.sectionSlug, id });
 ...
 */
export async function patchOrganisateurStageResourceStatusAction(input: {
  sectionSlug: string;
  id: string;
...
export async function deleteOrganisateurStageResourceAction(input: {
  sectionSlug: string;
  id: string;
}): Promise<{ ok: true }> {
  const ctx = await assertStageBureauContext();
