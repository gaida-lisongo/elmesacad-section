...
export type ValidationResourceRow = {
  id: string;
  designation: string;
  amount: number;
  currency: string;
...
function rowFromApi(raw: unknown): ValidationResourceRow | null {
...
function assertPayloadResourceIsValidation(raw: unknown) {
  const r = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
  const cat = String(r.categorie ?? "").toLowerCase();
  if (cat && cat !== "validation") {
    throw new Error("Cette ressource n'est pas une ressource « validation ».");
  }
}

export async function listGestionnaireValidationResourcesAction(input: {
  sectionSlug: string;
  page?: number;
  limit?: number;
  search?: string;
}): Promise<{ rows: ValidationResourceRow[]; total: number; page: number; limit: number }> {
  ...
  if (!upstream.ok) {
    throw new Error(pickErrorMessage(payload, "Impossible de charger les fiches de validation."));
  }
  ...
  const rows = data.map(rowFromApi).filter((x): x is ValidationResourceRow => x != null);
  return {
    rows,
    total,
...
export async function getGestionnaireValidationResourceAction(input: {
  sectionSlug: string;
  id: string;
}): Promise<
  ValidationResourceRow & {
...
> {
  ...
  assertPayloadResourceIsValidation(row);
...
export async function createGestionnaireValidationResourceAction(input: {
  sectionSlug: string;
  sectionCycle: string;
  designation: string;
  descriptionSections: DescriptionSectionInput[];
  ...
  try {
    row = await patchGestionnaireValidationResourceStatusAction({
...
export async function updateGestionnaireValidationResourceAction(input: {
  sectionSlug: string;
  id: string;
  sectionCycle: string;
  designation: string;
  ...
  await getGestionnaireValidationResourceAction({ sectionSlug: ctx.sectionSlug, id });
...
export async function patchGestionnaireValidationResourceStatusAction(input: {
  sectionSlug: string;
  id: string;
...
export async function deleteGestionnaireValidationResourceAction(input: {
  sectionSlug: string;
  id: string;
}): Promise<void> {
  const ctx = await assertGestionnaireSectionContext();
...
/**
 * Commandes étudiant pour une ressource `categorie: validation` — type commande `resultat` (service étudiant).
 */
export async function listValidationCommandesForResourceAction(input: {
  sectionSlug: string;
  resourceId: string;
  page?: number;
  limit?: number;
  ...
  assertPayloadResourceIsValidation(verifyRow);
